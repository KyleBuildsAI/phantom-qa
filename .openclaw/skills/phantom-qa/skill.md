...
name: phantom-qa
description: >
  Autonomous QA testing and fixing loop. Use this skill when the user asks to test,
  fix, debug, audit, or QA any software project. Also use when the user says
  "check if everything works", "find and fix bugs", "test the application", "run QA",
  or anything involving finding and fixing issues in a codebase. This skill enforces
  a mandatory verification loop -- the agent MUST verify every fix and continue until
...
If the PhantomQA test harness is available, run it:

```bash
# Check global OpenClaw skills location
node ~/.openclaw/workspace/skills/phantom-qa/scripts/test-harness.js

# Or if installed to project
